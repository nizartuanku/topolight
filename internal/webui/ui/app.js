...
(function () {
  ...
  async function pageReports(main) {
    ...
    // quick report
    const qp = h('select', { class: 'btn' }, ...[['24h', 'Last 24 h'], ['7d', 'Last 7 days'], ['30d', 'Last 30 days']].map(([v, l]) => h('option', { value: v, selected: v === '7d' }, l)));
    const qs = h('select', { class: 'btn' }, h('option', { value: '' }, 'All sites'), ...state.sites.map(s => h('option', { value: s.id }, s.name)));
