...
- **SNMP polls UDP/161 unless the credential says otherwise.** Set *UDP port* on an SNMP credential for agents that listen elsewhere — lab simulators, containers and homelab gear often use a high port because 161 needs privilege. The port belongs to the credential, so devices on different ports need a credential each.
